...
var (
	edgeFile = flag.String("image", "", "Input image.")
)
...
func EdgeDetect(img *image.Gray) (ximg, yimg ConvolutionImage) {
	...
	// ximg = ConvolveGray(img, [][]int8{{-1, 0, 1}, {-2, 0, 2}, {-3, 0, 3}, {-2, 0, 2}, {-1, 0, 1}})
	// yimg = ConvolveGray(img, [][]int8{{-1, -2, -3, -2, -1}, {0, 0, 0, 0, 0}, {1, 2, 3, 2, 1}})
	return
}
...
func main() {
	...
	baseName := strings.TrimSuffix(path.Base(*edgeFile), path.Ext(*edgeFile))

	gout, err := os.Create(fmt.Sprintf("%s--gray.png", baseName))
	xout, err := os.Create(fmt.Sprintf("%s--xsobel.png", baseName))
	yout, err := os.Create(fmt.Sprintf("%s--ysobel.png", baseName))
	mout, err := os.Create(fmt.Sprintf("%s--magnitude.png", baseName))
	aout, err := os.Create(fmt.Sprintf("%s--angle.png", baseName))
